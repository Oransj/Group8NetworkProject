...
A potential improvement regarding security would be to use our own MQTT broker with TLS support, as using only RSA with no padding like sha-256 is not very secure. Unfortunately the Python and Java cryptography libraries were not especially compatible and padding could not be implemented until the deadline.
