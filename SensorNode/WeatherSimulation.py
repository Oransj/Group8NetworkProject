...
class mqtt_client:
    ...
    def format_to_json(self, weather : weather) -> str:
        ...
        json_string = json.dumps({"Reading1": {"Time": {"ms" : t_ms}, "Temperature": {"celsius" : round(weather.temperature, 2)}, "Precipitation": {"mm" : round(weather.precipitation, 2)}, "AirPressure": {"hPa" : weather.pascal}, "Lux": {"lux" : round(weather.lux, 5)}, "WindSpeed": {"m/s" : round(weather.wind_speed, 2)}, "WindDirection": {"degrees" : round(weather.winddir, 2)}}})
        print(json_string)
        return json_string
